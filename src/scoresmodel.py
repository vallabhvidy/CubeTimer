...
json_path = scores_file_path

class ScoresDB(GObject.Object):
    __gtype_name__ = "ScoresDB"
    ...
    def load(self):
        try:
            ...
            # create index on session, score_idx
            self.c.execute("""
    ...
    def migrate(self):
        ...
        try:
            with open(json_path, 'r') as scores_file:
                sessions = json.load(scores_file)
                version = sessions.get("version", 1)
                if version == 3:
                    return
                self.set_last_session(sessions.get("last-session", _("Session 1")))
